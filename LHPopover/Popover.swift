...
//
//  Bugfix by luhak on 16/1/18
//  The auther`s pod name is 'Popover'
//  My bugfix pod name is 'LHPopover'
//  The bugfix and optimization version Copyright (c) 2018 luhao.
//  All rights reserved.
...
open class Popover: UIView {
    ...
    open var animationType: AnimationType = .pop
...
extension Popover {
    ...
    //MARK: expand动画
    private func expandShow() {
        self.transform = CGAffineTransform(scaleX: 1.0, y: 0.0)
        UIView.animate(withDuration: self.animationIn,
                       animations: {
                        self.transform = CGAffineTransform.identity
        }) { (complete) in
            if complete {
    ...
    private func expandDismiss() {
        self.transform = CGAffineTransform.identity
        UIView.animate(withDuration: self.animationIn,
                       animations: {
                        self.transform = CGAffineTransform(scaleX: 1.0, y: 0.0001)
                        self.blackOverlay.alpha = 0
        }) { (complete) in
            self.contentView.removeFromSuperview()
            self.blackOverlay.removeFromSuperview()
